...
DOMAIN_METADATA = {
    'healthcare': {
        'lineage': [
            ...
            ('CSV:Vitals','Vitals_Export.csv','patient_id','Fact_Clinical_Encounter','patient_sk','SK Lookup via Dim_Patient'),
            ('CSV:Vitals','Vitals_Export.csv','systolic','Fact_Clinical_Encounter','systolic_bp','Extract leading digits'),
            ('CSV:Vitals','Vitals_Export.csv','diastolic','Fact_Clinical_Encounter','diastolic_bp','Extract trailing digits'),
            ('EMR:Epic','Medications','medication_id','Dim_Medication','source_medication_id','Type 1 SCD; direct replace'),
            ('EMR:Epic','Medications','generic_name','Dim_Medication','generic_name','Normalize to uppercase'),
            ('CSV:Prescriptions','Prescription_History.csv','medication_id','Fact_Claims','medication_sk','SK Lookup via Dim_Medication'),
            ('API:Pharmacy','clearinghouse.io/rx','rx_number','Fact_Claims','rx_number','Direct Mapping'),
            ('API:Pharmacy','clearinghouse.io/rx','insurance_status','Fact_Claims','status','Direct string map'),
        ],
        'catalog': [
            ...
            ('Dim_Medication','Master drug and NDC code dictionary','Clinical','Pharmacy Director','EMR_TO_DW_MEDS','Monthly',False),
            ...
            ('Medications','Source EMR Drug Dictionary','Clinical EMR','EMR Admin','N/A','Real-time',False),
            ('Lab_Results.csv','LIS system bulk flat file export','Flat File','Lab Supervisor','N/A','Batch',True),
            ('Vitals_Export.csv','Nursing vitals telemetry export','Flat File','Nursing Admin','N/A','Batch',True),
            ('Prescription_History.csv','Pharmacy fill history export','Flat File','Pharmacist','N/A','Batch',True),
            ('clearinghouse.io/claims','Insurance claims clearinghouse REST endpoint','API','Billing Director','N/A','Streaming',True),
            ('clearinghouse.io/rx','Pharmacy benefit manager REST endpoint','API','Pharmacy Director','N/A','Streaming',True),
        ],
        'reports': [
            ...
            ('Chronic Care Management Dashboard','Chief Medical Officer','Dim_Medication','Weekly',
             'Top 10 Prescribed Drugs, Medication Adherence Rate (%), Adverse Event Count'),
            ('Chronic Care Management Dashboard','Chief Medical Officer','Fact_Claims','Weekly',
             'Total Rx Cost ($), Avg Copay ($), Generic Fill Rate (%)'),
        ...
        'audit_tables': [
            'Dim_Patient','Dim_Provider','Dim_Facility','Dim_Diagnosis','Dim_Medication',
            'Fact_Clinical_Encounter','Fact_Claims','Lab_Results.csv','clearinghouse.io/claims',
            'Vitals_Export.csv','Prescription_History.csv','clearinghouse.io/rx',
            'Patients','Providers','Facilities','Diagnoses','Medications',
        ],
        'bi_users': [
